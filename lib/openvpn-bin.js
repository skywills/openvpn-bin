    ...
    return new Promise(function(resolve, reject) {
        fs.existsAsync(args.config)
            ...
            .then(function(setargs) {
                let tempArgs = setargs;
                let firstExecute = openvpnpath;
                // using sudo note: please use this carefully as inject openvpnpath to run other executable
                if(args?.requiredAdmin && (process.platform =="darwin" || process.platform == "linux")) {
                    firstExecute = "sudo";
                    tempArgs = [openvpnpath, ...tempArgs];
                }
                return child_process.execFileSync(firstExecute,tempArgs);
            })
            .then(function() {
                resolve({
                    port: args.port,
